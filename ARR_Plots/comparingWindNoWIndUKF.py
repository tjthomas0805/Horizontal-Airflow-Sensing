...
# def main():
#     # Data path
#     filepath = r"C:\Users\ltjth\Documents\Research\UKF_Data\CF_ARR_Square_EKF2.0ms_4.5m_FAN1.csv"
#
#     print("=" * 60)
...
"""
Wind Comparison Analysis
Runs UKF on two datasets (with and without wind) and compares results.
Estimates wind as the difference between UKF airflow estimate and ground truth velocity.
"""
...
class AirflowUKF:
    ...
    def _fx(self, x, dt):
        return x  # Random walk (acceleration disabled)
...
def plot_comparison(results_no_wind, results_wind, title_no_wind="No Wind", title_wind="With Wind"):
    ...
    ax = axes[1, 0]
    # ax.plot(results_no_wind['time'], results_no_wind['wind_x'],
    #         'b-', lw=1.5, label='Wind X', alpha=0.8)
    # ax.plot(results_no_wind['time'], results_no_wind['wind_y'],
    #         'r-', lw=1.5, label='Wind Y', alpha=0.8)
    ...
    ax.set_ylim([0, 1.25])
    ax.set_ylabel('Wind (m/s)')
    #ax.set_title(f'{title_no_wind}: Estimated Wind (should be ~0)')
    ax.legend()
    ax.grid(True, alpha=0.3)

    wind_mean = np.mean(results_no_wind['wind_mag'])
    wind_std = np.std(results_no_wind['wind_mag'])
    # ax.text(0.02, 0.98, f'Mean: {wind_mean:.3f} m/s\nStd: {wind_std:.3f} m/s',
    #         transform=ax.transAxes, fontsize=10, verticalalignment='top',
    #         bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))

    # Bottom right: With wind - wind estimate
    ax = axes[1, 1]
    # ax.plot(results_wind['time'], results_wind['wind_x'],
    #         'b-', lw=1.5, label='Wind X', alpha=0.8)
    # ax.plot(results_wind['time'], results_wind['wind_y'],
    #         'r-', lw=1.5, label='Wind Y', alpha=0.8)
    ...
    ax.set_ylim([0, 1.25])
    ax.set_ylabel('Wind (m/s)')
    #ax.set_title(f'{title_wind}: Estimated Wind')
    ax.legend()
    ax.grid(True, alpha=0.3)

    wind_mean = np.mean(results_wind['wind_mag'])
    wind_std = np.std(results_wind['wind_mag'])
    # ax.text(0.02, 0.98, f'Mean: {wind_mean:.3f} m/s\nStd: {wind_std:.3f} m/s',
    #         transform=ax.transAxes, fontsize=10, verticalalignment='top',
    #         bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
...
def main():
    ...
    # =========================================================================
    no_wind_file = r"C:\Users\ltjth\Documents\Research\UKF_Data\CF_ARR_Line_EKF2.0ms_4.5m2.csv"  # <-- no wind file
    wind_file = r"C:\Users\ltjth\Documents\Research\UKF_Data\CF_ARR_Line_EKF2.0ms_4.5m_FAN1.csv"  # <-- wind file

    print("=" * 60)
    print("WIND COMPARISON ANALYSIS")
    print("=" * 60)

    # UKF parameters (from your tuned values)
    Q_airflow = 0.005
    R_odom = 0.003
    R_magnetic = np.array([
        [0.72422324, 0.33065],
    ...
    print(f"\nNo Wind:")
    print(
        f"  Wind estimate: {np.mean(results_no_wind['wind_mag']):.3f} ± {np.std(results_no_wind['wind_mag']):.3f} m/s")
    print(f"\nWith Wind:")
